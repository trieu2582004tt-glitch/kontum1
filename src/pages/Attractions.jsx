...
function LocationCard({ location, checkins, onAddCheckin, gpsCoords, onIncrementViews }) {
  ...
  return (
    <article className="card">
      ...
      <div className="card-content-box">
        ...
        <p className="small price-range">Vé tham quan: {location.priceRange}</p>

        <div className="card-rating" style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {stats.count > 0 ? (
            <span className="rating-badge">⭐ {stats.average} ({stats.count} lượt check-in)</span>
          ) : (
            <span className="rating-badge no-rating">⭐ Chưa có lượt check-in</span>
          )}
        ...
        <div className="card-footer-actions">
          ...
          <button type="button" className="btn-toggle-reviews" onClick={() => {
            ...
            {isExpanded ? 'Đóng' : `Kỷ niệm (${stats.count})`}
          </button>
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="card-reviews-section">
          <h4>Nhật ký Check-in & Trải nghiệm</h4>
          {reviews.length === 0 ? (
            <p className="no-reviews-text">Chưa có ai lưu dấu kỷ niệm ở đây. Hãy check-in đầu tiên!</p>
          ) : (
          ...
          <ReviewForm
            ...
            placeholder="Chia sẻ cảm nhận, kinh nghiệm du lịch tại điểm này..."
...
export default function Attractions() {
  ...
  useEffect(() => {
    fetch('/api/locations?type=attraction')
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="page-shell">
      <header className="hero">
        <div>
          <p className="eyebrow">Địa danh kỳ vĩ</p>
          <h1>Danh Lam Thắng Cảnh Kon Tum</h1>
          <p>
            Khám phá những điểm check-in lịch sử, núi rừng hoang sơ và thác nước kỳ vĩ từ TP Kon Tum thơ mộng lên tới Măng Đen sương mờ.
          </p>
        </div>
      </header>
      ...
      <main className="content-container">
        <section className="section-intro">
          ...
          <div className="quick-tags-wrapper" style={{ marginTop: '16px', marginBottom: '24px' }}>
            ...
            <div className="quick-tags">
              {['Tất cả', 'Văn hóa & Lịch sử', 'Thiên nhiên', 'Hồ & Công viên', 'Điểm check-in'].map((cat) => (
                <button
                  key={cat}
                  type="button"
          ...
          <h2>Danh sách danh lam thắng cảnh ({processedLocations.length})</h2>
          {processedLocations.length === 0 ? (
            <p className="no-comments-text">Không tìm thấy địa điểm du lịch nào phù hợp.</p>
          ) : (
        ...
        <section className="section-checkins">
          <h2>Nhật ký check-in danh thắng mới nhất ({filteredCheckins.length})</h2>
            ...
            <div className="checkin-list">
              {filteredCheckins.map((checkin) => (
                <article key={checkin.id} className="checkin-card">
                  ...
                  <p>{checkin.comment}</p>
                  <p className="small">Địa điểm: {checkin.restaurant_name}</p>
                </article>
              ))}
            </div>
          )}
